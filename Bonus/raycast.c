...
/*   Updated: 2025/02/23 12:18:40 by cmasnaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_min_distance(t_data *data, double distance)
{
	...
	{
		1 && (data->ray->view = 'v') && (data->direction_e = 1);
		data->texture = data->mlx->ea_wall;
		data->ray->left && (data->direction_e = 0) && (data->direction_w = 1)
				&& (data->texture = data->mlx->we_wall);
		data->map->v_door && (data->texture = data->mlx->door)
				&& (data->map->h_door = 0);
	}
	if (distance < data->ray->distance)
	{
		1 && (data->ray->view = 'h') && (data->direction_s = 1);
		data->ray->distance = distance;
		data->texture = data->mlx->so_wall;
		data->ray->up && (data->texture = data->mlx->no_wall)
			&& (data->direction_s = 0) && (data->direction_n = 1);
		data->map->h_door && (data->texture = data->mlx->door)
			&& (data->map->v_door = 0);
	}
}
